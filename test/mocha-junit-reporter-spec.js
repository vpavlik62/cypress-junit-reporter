...
describe('mocha-junit-reporter', function () {
    ...
    describe('when "outputs" option is specified', function () {
        it('adds output/error lines to xml report', function (done) {
            ...
            test._testConfig = {};
            test._testConfig.consoleOutputs = ['hello', 'world'];
            test._testConfig.consoleErrors = ['typical diagnostic info', 'all is OK'];
        ...
        it('does not add system-out if outputs/errors were empty', function (done) {
            ...
            var test = createTest('has outputs');
            test._testConfig = {};
            test._testConfig.consoleOutputs = [];
            test._testConfig.consoleErrors = [];
    ...
    describe('when "attachments" option is specified', function () {
        it('adds attachments to xml report', function (done) {
            ...
            test._testConfig = {};
            test._testConfig.attachments = [filePath];
        ...
        it('includes both console outputs and attachments in XML', function (done) {
            ...
            test._testConfig = {};
            test._testConfig.attachments = [filePath];
            test._testConfig.consoleOutputs = ['first console line', 'second console line'];
    ...
    describe('when "includeSpecFile" option is specified', function () {
        it("prepends [specFilePath] to full test title (including suites)", function (done) {
            var specName = "test.spec.ts";
            var reporter = createReporter({ includeSpecFile: true });
            var test = createTest('includes specFile');
            test.file = specName;

            var suite = Suite.create(reporter.runner.suite, 'with specFile');
            suite.addTest(test);

            runRunner(reporter.runner, function () {
                expect(reporter._testsuites[1].testsuite[0]._attr.name).to.equal(suite.title);
                expect(reporter._testsuites[1].testsuite[1].testcase).to.have.lengthOf(1);
                expect(reporter._testsuites[1].testsuite[1].testcase[0]._attr.name).to.equal(`[${specName}] ${test.fullTitle()}`);

                done();
            });
        });

        it("prepends [specFilePath] to test name only (testCaseSwitchClassnameAndName=true)", function (done) {
            var specName = "test.spec.ts";
            var reporter = createReporter({ includeSpecFile: true, testCaseSwitchClassnameAndName: true });
            var test = createTest('includes specFile');
            test.file = specName;
            ...
            runRunner(reporter.runner, function () {
                expect(reporter._testsuites[1].testsuite[0]._attr.name).to.equal(suite.title);
                expect(reporter._testsuites[1].testsuite[1].testcase).to.have.lengthOf(1);
                expect(reporter._testsuites[1].testsuite[1].testcase[0]._attr.name).to.equal(`[${specName}] ${test.title}`);
